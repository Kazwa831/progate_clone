...
## 2026-07-30: モバイル対応 段階1（全画面共通の土台）

スマホでの表示を調査したうえで、影響範囲が広く変更の小さい部分から着手した。
判定ロジック・進捗保存・APIには一切手を触れていない。

### 事前調査でわかったこと（iPhone SE 320px / iPhone 14 390px で実測）
良かった点として、**横スクロールはどの画面でも発生しておらず**、viewportメタタグも
正しく入っていた。学習画面のサイドバーも既に `hidden md:flex` で隠れていた。

問題は次の4点。
1. **ヘッダーが全画面で崩れる**。ヘッダーの箱は57pxなのに、中の項目が
   77pxまで伸びていた（文字が折り返して「コ/ー/ス」と縦に割れる）
2. **学習画面が機能していない**（段階2で対応）。エディタが85px＝約4行、
   演習スライドでは**プレビューiframeの高さが0**で、書いたコードの結果が見えない。
   解説スライドでは「次へ」がy=608と画面外（画面は568px）
3. タップ領域が小さい（ダッシュボードの「続きから →」が74×22px）
4. ダッシュボードが縦に長い（段階3で対応）

なお、トップのコースカードのタイトルリンクも23px高と計測されたが、
これは `::before` でカード全体が当たり判定になっているため**実質問題ない**。
計測値だけを見ると誤検知するので、検証スクリプト側で `::before` の有無を見て
除外するようにした。

### 段階1で実施したこと
- **ヘッダー**: 320px幅では全項目が収まらないため、
  - 狭い画面では「コース」リンクを出さない（**ロゴがコース一覧へのリンクを
    兼ねている**ので、辿れなくなる導線ではない）
  - 「学習ダッシュボード」は狭い画面では「記録」と短く表示
  - 各項目に `whitespace-nowrap`、左右の余白を `px-4 sm:px-6` に
  - タップ領域を44px以上に（`min-h-11 sm:min-h-0`）。
    **`sm:min-h-0` を付けてデスクトップの見た目は据え置き**
- **`h-screen` → `h-dvh`**: iOS SafariのURLバーで100vhが実表示領域を超える問題への対応。
  特に学習画面は画面固定レイアウトなので影響が大きい
- **タップ領域**: ダッシュボードの「続きから」「始める」「コース名」を44px以上に

### 動作確認結果（すべて実測）
- `npm run lint` / `npm run build` 成功
- **ヘッダーのはみ出しが解消**（iPhone SE / iPhone 14 / iPad Mini、
  ログイン中・未ログインの計4パターンで、項目の高さがヘッダー内に収まることを確認）
- **タップ領域44px未満が0件に**（トップ・コース詳細・ダッシュボード・ログイン）
- 横スクロールは引き続き発生なし
- **デスクトップ（1280px）の回帰なし**: ヘッダー項目の高さは23/34pxで変更前と同じ、
  学習画面のエディタ376px・プレビュー377pxも変更前と同じ。ライト/ダーク両方で確認
- コンソールエラー: 0件
- 検証用アカウントは削除済み

### 次回やること（段階2: 学習画面のタブ化）
ユーザーと相談し、4案（タブ切替 / 縦スクロール / シート / 閲覧専用）から
**タブ切替（解説・コード・結果）**を採用することにした。
- `md未満`のみタブ表示、`md以上`は現行の3ペインを維持
- 下部固定の操作バー（実行して確認する・前へ・次へ）で画面外に消える問題を解消
- サイドバーの代わりに `☰` で開くレッスン一覧のドロワーを追加
- 正解時は自動で「結果」タブへ切り替える

---

## 2026-07-30: モバイル対応 段階2（学習画面のタブ化）

調査で最も深刻だった学習画面を作り直した。判定ロジック・進捗保存・APIには触れていない。

### 採用した構成
ユーザーと4案（タブ切替 / 縦スクロール / シート / 閲覧専用）を比較し、
**タブ切替**を採用した。決め手は、各ペインが画面高さを使えるためエディタが実用的な
高さになること、iOSでキーボードが出ても破綻しにくいこと。

```
md未満: [☰ レッスン名] / [解説|コード|結果] / 選択中のペイン / [実行して確認する][前へ][次へ]
md以上: [サイドバー] [解説 | エディタ/プレビュー]   ← 従来のまま
```

### 実装の要点
- **DOMは1つのまま、表示/非表示だけで切り替える。** デスクトップ用のクラスには
  手を入れていないため、`md`以上の3ペインは構造ごと維持される
- 新規: `LessonTabs.tsx`（タブ）、`LessonActionBar.tsx`（下部固定の操作バー）、
  `LessonDrawer.tsx`（レッスン一覧のドロワー）
- ドロワーは**サーバーコンポーネントの`LessonSidebar`を`children`として受け取る**形にした。
  開閉状態のためだけにクライアント化する範囲を最小に留められる
- `SlidePanel`内の前へ/次へは `hidden md:block` に。狭い画面では操作バーが担当するため、
  同じ操作が2か所に出ないようにした
- スライドを移動したら「解説」タブに戻し、実行したら「結果」タブへ自動で切り替える

### 実装中に気づいて直した点
1. **HTML/CSSの判定はプレビューのiframeのDOMを読む**ため、コードタブ（プレビューが
   非表示）のまま判定すると結果が変わりうる。操作バーからの実行では、先に結果タブへ
   切り替え、描画が反映されてから判定するようにした（`requestAnimationFrame`を2回）。
   4コースすべてで判定が通ることを実測で確認済み
2. 実装後のスクリーンショットで、結果タブに「実行して確認する」が
   **操作バーとResultCheckerで2つ並んでいた**。計画時は許容と考えていたが、
   実際に見ると明らかに冗長だったため、結果タブでは操作バー側に出さないようにした

### 動作確認結果（すべて実測 / iPhone SE 320px）
| 項目 | 変更前 | 変更後 |
|---|---|---|
| エディタの高さ | 85px（約4行） | **346px** |
| プレビューの高さ | 演習スライドで**0px** | **347px** |
| 「次へ」の位置 | y=608（画面外） | **y=514（常に画面内）** |
| レッスン一覧 | 到達手段なし | **☰ から28件表示・遷移可** |

- 4コース（HTML/CSS・JavaScript・Python・SQL）すべてでスマホから判定が通り、
  実行後に結果タブへ切り替わることを確認
- **デスクトップ1280pxの回帰なし**: エディタ376px・プレビュー377px・サイドバー288pxで
  変更前と完全に一致。タブバーと操作バーは非表示、スライド内の前へ/次へは表示
- 進捗の保存・復元（リロード後のスライド位置と書きかけコード）が従来どおり動作
- ライト/ダーク両モードでスクリーンショットを確認
- `npm run lint` / `npm run build` 成功、コンソールエラー0件

### 検証時の補足
デスクトップ確認で「次へ」が非表示と出たが、テスト側の問題だった。再開位置が最終
スライドだったため実際のラベルが「次のレッスンへ →」で、`/次へ/` という正規表現に
一致していなかった。DOMを直接確認したところ、スライド内のナビは `visible: true`、
モバイル操作バーの複製は `visible: false` で正しく動作していた。

### 次回やること（段階3: ダッシュボード・コース詳細の詰め）
- 統計カードをモバイルで2列にし、縦の長さを短縮する
- コース別進捗の行レイアウトを整える

---

## 2026-07-30: モバイル対応 段階3（ダッシュボード・コース詳細の詰め）

モバイル対応の仕上げ。判定ロジック・進捗保存・APIには触れていない。

### ダッシュボード
- 統計カードは `sm:grid-cols-2`（640px以上で2列）だったため**320pxでは1列に積まれていた**。
  `grid-cols-2` を基点にして、狭い画面でも2列に並ぶようにした
- 主役カードの余白を `p-5 sm:p-7`、セクション間隔を `mt-10 sm:mt-14` に
- **完了レッスン一覧で、右側の「コース名・章名」が幅を奪ってレッスン名が切れていた**。
  狭い画面では補足を下の行に回し、レッスン名が全幅を使えるようにした
  （これは事前に提示したファイル一覧に無かったが、検証中に見つけたため段階3の範囲として対応した）

### コース詳細
- 「最初のレッスン」カードで `0%`・`0 / 26 完了`・ボタンが窮屈に重なっていた。
  狭い画面では「0% 0 / 26 完了」を1行にまとめ、ボタンを全幅にして縦に積むようにした
- レッスン行の状態ラベル（未着手/学習中/完了）が幅を奪い、
  **レッスン名が途中で切れていた**（「ナビゲーションを横並びに…」）。
  狭い画面では文字を隠し、左の丸マーカーで状態を示すようにした

### 状態ラベルの隠しかた
`aria-label` をマーカーに付ける案もあったが、**`sr-only sm:not-sr-only`** を採用した。
- 実テキストがそのまま読み上げられる（別途ラベルを書き起こす必要がない）
- 広い画面でラベルが見えているときに、読み上げが二重にならない

### 数値タイルの文字サイズ
2列にすると1枚あたり104pxになり、`type-headline`（24px）では
**「2026/12/30」が137pxとなってはみ出した**。
「タイポグラフィは globals.css のトークンに集約する」という既存ルールに従い、
コンポーネント側で文字サイズを指定するのではなく `.type-stat-value` を新設した
（狭い画面17px / 640px以上は従来どおり24px）。

### 動作確認結果（すべて実測）
| 画面 | 変更前 | 変更後 |
|---|---|---|
| ダッシュボード全長(SE) | 2230px | **1820px** |
| ダッシュボード全長(iPhone 14) | — | **1780px** |
| コース詳細の全長(SE) | 2740px | 2671px |

- 「2026/12/30」が枠内(104px/104px)に収まることを確認
- レッスン名が切れなくなったことを確認（完了レッスン一覧・コース詳細とも）
- 状態ラベルは実測で 1×1px（視覚的に隠れている）かつDOMに存在＝読み上げには残る
- 横スクロールなし・タップ領域44px未満0件を維持
- **デスクトップ1280pxの回帰なし**: 統計カードは4列(各200px)、数値24px、カード余白20px、
  状態ラベル26件が表示、次のレッスンカードは横並び(row)。ライト/ダーク両方で確認
- `npm run lint` / `npm run build` 成功、コンソールエラー0件
- 検証用アカウントと投入した確認用データは削除済み

### 検証時の補足
検証スクリプトが「完了」「未着手」を"文字が収まっていない要素"として報告したが、
これは `sr-only` が要素を1pxにクリップする仕様によるもので、誤検知だった。

### モバイル対応（段階1〜3）の完了
これでスマホでも全機能が使える状態になった。今後の候補は章末の復習機能、
レッスン数の拡充など。
